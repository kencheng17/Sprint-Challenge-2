...
// nested function can access the variable internal because both `internal` and `nestedfunction` are on the same level scope
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(zooAnimals){
    var arr = []
    zooAnimals.forEach(animal => arr.push(`name: ` + animal[`animal_name`] + `, scientific: ` + animal[`scientific_name`]))
    return arr
  }
  ...
  */
  ...
  function lowerCaseNames(zooAnimals){
    var arr = zooAnimals.map(animal => animal[`animal_name`].toLowerCase())
    return arr
  }
  ...
  */
  ...
  function USApop(zooAnimals){
    var res = zooAnimals.reduce((prev, curr) => prev + curr [`population`], 0)
    return res
  }
...
var cuboidTwo = new CuboidMakerTwo({'length': 4, 'width': 5, 'height': 5})
